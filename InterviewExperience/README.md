...
> 是重写(覆盖)，**函数重写是子类和父类之间的关系，是垂直关系**，具体编写为：子类的某个函数重写了父类对应的虚函数
>
> 规则：
>
> - 在一个继承体系中，一个为父类虚函数，一个为子类虚函数
> - 除了协变和析构函数，两个函数必须完全相同(返回值类型，函数名，形参列表)
> - 静态成员函数不能重写，因为调用虚函数需要用到this指针

**overload**

> 是重载，**函数重载是在同一个作用域中，是水平关系**，具体编写为：函数名相同，形参列表不同
>
> 规则：
>
> - 相同作用域
